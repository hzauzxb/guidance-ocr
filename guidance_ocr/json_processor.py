    ...
    def __call__(self, input_ids, scores):
        ...
        for token in order_indexes:
            if self.status == self.SPECIAL_STATUS:
                ...
                # 判断token是否在 key 中
                if self.SPECIAL2KEY:
                    is_key_valid, possible_avi_nodes = is_valid(self.key_all_nodes, text)
                    if is_key_valid:
                ...
                if self.SPECIAL2VALUE:
                    ...
                    is_ocr_valid, possible_avi_nodes = is_valid(self.ocr_all_nodes, text)
                    if is_ocr_valid:
                ...
                # 这里会出现换行的情况
                if len(self.avi_nodes) == 0:
                    self.avi_nodes = self.ocr_all_nodes
                
                # 判断是否在value中
                text = self.tokenizer.decode(token)
                is_value_vaild, possible_avi_nodes = is_valid(self.avi_nodes, text)
                if is_value_vaild:
                    self.avi_nodes = possible_avi_nodes
                    valid_token = token
                    break
