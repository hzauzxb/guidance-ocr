...
from guidance_ocr import get_ocr_processor, OCRAssistModel
...
for key in ['个人自付', '票据代码', '票据号码', '开票日期', '卫生材料费', '校验码']:
    ...
    conversation = [
        {
            ...
            "content": [
                ...
                {"type": "text", "text": f"请给出图中的{key}，只需输出答案不要输出多余内容"},
            ],
        }
    ]
    ...
    generated_ids = model.generate(
        **inputs,
        logits_processor = [ocr_processor],
        max_new_tokens=128
    )
    en = time.time()
    res = tokenizer.decode(generated_ids[0][inputs['input_ids'].shape[1]:])
    print(f'guided not speculative\t{res}\ttime:{en - st}')
    ...
    en = time.time()
    res = tokenizer.decode(generated_ids[0][inputs['input_ids'].shape[1]:])
    print((f'guided speculative\t{res}\ttime:{en - st}'))