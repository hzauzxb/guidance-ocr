...
for extract_key in ['个人自付', '票据代码', '票据号码', '开票日期', '卫生材料费', '校验码']:
    prompt = '<|im_start|>system\n你是由上海人工智能实验室联合商汤科技开发的书生多模态大模型，英文名叫InternVL, 是一个有用无害的人工智能助手。<|im_end|><|im_start|>user\n<img>'
    prompt = prompt + f'<IMG_CONTEXT>' * num_patches_list * 256 + f'</img>请给出图中的{extract_key}，请只输出{extract_key}不要输出多余内容<|im_end|><|im_start|>assistant\n{extract_key}：'
    ...
    with warp_model(
            ...
            text_list = text_list, 
            model_type = 'internvl2', allow_texts = [],
            accelerate = True, max_cache = 20, top_k = 100
        ) as ocr_guided_model:
